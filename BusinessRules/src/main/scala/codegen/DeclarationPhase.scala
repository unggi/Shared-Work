...
import codegen.symbols.{MatchScope, ModelReferenceSymbol, SymbolTableBuilder}
      ...
      case path: DottedModelPathContext =>
        ...
        val alias =
          if (ctx.modelReferenceWithAlias.alias != null)
            ...
            components.mkString(".")
        symbolTable.openScope(new MatchScope(symbolTable.scope))
        val entry = new ModelReferenceSymbol(alias, components)
